...
/**
 * Class component
 * @package core\database\driver\PDO
 */
class PDO extends AQueryConstructor implements IQueryConstructor
{
    ...
    protected function __construct(array $config = Array())
    {
        ...
        try {
            $this->config['driver']     = $config['driver'] ?? $this->config['driver'];
            $this->config['host']       = $config['host'] ?? $this->config['host'];
            $this->config['port']       = $config['port'] ?? $this->config['port'];
            $this->config['db']         = $config['db'] ?? $this->config['db'];
            $this->config['name']       = $config['name'] ?? $this->config['name'];
            $this->config['pass']       = $config['pass'] ?? $this->config['pass'];
            $this->config['character']  = $config['character'] ?? $this->config['character'];
            $params = array(
            ...
            $this->connect->exec("SET character_setconnection = {$this->config['character']};" );
            $this->connect->exec("SET character_set_client = {$this->config['character']};" );
            $this->connect->exec("SET character_set_results = {$this->config['character']};" );
            $this->connect->exec('SET NAMES ' . $this->config['character']);;
    ...
    {
        $result = $this->selectGenerator($table, $fields, $where, $order, $limit, $group, $having);
        $query  =   $this->getConnect()->prepare($result['sql']);
        if ($query === false) {
            $execute = [];
