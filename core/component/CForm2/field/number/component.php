...
namespace core\component\CForm2\field\number;
...
/**
 * Class component
 * @package core\component\CForm\field\number
 */
class component extends CForm2\AField implements CForm2\IField
{
	...
	public function init()
    {
        if (isset(self::$data['id'])) {
            ...
            $id = self::$data['id'];
            $this->addAnswerID("number-field-{$field}-id-{$id}");
            $this->addAnswerClass('number');
            self::$config['controller']::setCss(self::getTemplate('css/number.css', __DIR__));
        }
    }
    ...
    public function view()
    {
        ...
        if (isset($this->componentSchema[self::$mode], $this->componentSchema[self::$mode]['align'])) {
            switch ($this->componentSchema[self::$mode]['align']) {
                case 'left':
                    $this->addAnswerClass('number-left');
                    break;
                case 'center':
                    $this->addAnswerClass('number-center');
                    break;
                case 'right':
                    $this->addAnswerClass('number-center');
                    break;
