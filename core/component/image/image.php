...
class image
{
     ...
     * @return string
     */
    public static function image(string $uri, array $stack = []): string
    {
    ...
    {
        try {
            if (null === $this->uri || !\is_string($this->uri)) {
                return '';
            }
            ...
            $imagePath = dir::getDR(true) . $originalURI;
            if (!file_exists($imagePath)) {
                $imagePath = dir::getDR(false) . $originalURI;
                if (!file_exists($imagePath)) {
                    return '';
                }
            }

            $imageExtension = array_reverse(explode('.', $originalURI))[0];
            $thumbnailName = md5($originalURI . base64_encode(serialize($this->stack)));
            ...
            foreach ($this->stack as $value) {
                $method = 'image' . ucfirst($value['action'] ?? '');
